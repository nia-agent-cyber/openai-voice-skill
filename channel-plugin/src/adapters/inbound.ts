...
import { voiceConfigAdapter, type VoiceAccountConfig } from "./config.js";
...
const callerHistory = new Map<string, {
  name?: string;
  callCount: number;
  lastCallAt: string;
  notes?: string;
...
// Default logger
const defaultLogger: InboundLogger = {
 ...
 */
export function authorizeInboundCall(
  callerPhone: string,
 ...
 */
function checkAllowlist(phone: string, allowFrom: string[]): string | null {
 ...
 */
export function buildInboundSessionContext(
  event: InboundCallEvent,
  logger: InboundLogger = defaultLogger
 ...
 * Update caller notes
 */
export function updateCallerNotes(
  callerPhone: string,
  notes: string
): void {
  const normalized = normalizePhoneNumber(callerPhone);
 ...
 */
export function recordMissedCall(
  from: string,
 ...
 */
export function getPendingMissedCalls(): MissedCall[] {
  return missedCalls.filter(
    (call) => !call.callbackScheduled && call.hasVoicemail
  );
}
 ...
 */
export function markCallbackScheduled(timestamp: string): boolean {
 ...
 */
export function generateRejectTwiml(
  reason: string,
  recordVoicemail: boolean = true
): string {
  if (recordVoicemail) {
 ...
 * @returns TwiML XML string
 */
export function generateAcceptTwiml(sipUri: string): string {
  return `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Dial>
    <Sip>${sipUri}</Sip>
...
export async function handleInboundCall(
  ...
  logger.info(
    `Incoming call: ${maskPhoneNumber(event.from)} → ${maskPhoneNumber(event.to)}`
  );
  ...
  if (!authResult.authorized) {
    logger.warn(
      `Call rejected: ${maskPhoneNumber(event.from)} - ${authResult.reason}`
    );
  ...
  logger.info(
    `Call authorized: ${maskPhoneNumber(event.from)} → session ${sessionContext.sessionKey}`
  );
 ...
 */
export function getCallerHistory(callerPhone: string) {
  const normalized = normalizePhoneNumber(callerPhone);
 ...
 */
export function getAllKnownCallers(): Array<{
  phone: string;
  name?: string;
  callCount: number;
  lastCallAt: string;
 ...
 */
export function getRecentMissedCalls(limit: number = 10): MissedCall[] {
  return missedCalls
    .slice(-limit)
    .reverse()
    .map((call) => ({
